...
export default function OwnerAvailabilityCalendar({ vehicles }: { vehicles: any[] }) {
  ...
  useEffect(() => {
    ...
    void Promise.resolve().then(() => loadData(selectedVehicleId));
  }, [selectedVehicleId]);

  async function addBlock(event: React.FormEvent) {
    event.preventDefault(); if (!selectedVehicleId) return;
    const response = await fetch(`/api/vehicles/${selectedVehicleId}/availability`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ startDate, endDate, reason: 'OWNER_BLOCK' }) });
  ...
  async function removeBlock(blockId: string) { const response = await fetch(`/api/vehicles/${selectedVehicleId}/availability`, { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ blockId }) }); if (response.ok) setBlocks((current) => current.filter((block) => block.id !== blockId)); else setMessage((await response.json()).error); }
  async function addRule(event: React.FormEvent) { event.preventDefault(); const response = await fetch(`/api/vehicles/${selectedVehicleId}/pricing`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: ruleName, startDate: ruleStart, endDate: ruleEnd, pricePerDay: rulePrice }) }); const data = await response.json(); if (response.ok) { setRules((current) => [...current, data.rule].sort((a, b) => a.startDate.localeCompare(b.startDate))); setRuleStart(''); setRuleEnd(''); setRulePrice(''); setMessage('Tarifa guardada'); } else setMessage(data.error); }
  async function removeRule(ruleId: string) { const response = await fetch(`/api/vehicles/${selectedVehicleId}/pricing`, { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ruleId }) }); if (response.ok) setRules((current) => current.filter((rule) => rule.id !== ruleId)); else setMessage((await response.json()).error); }
  async function importCalendar(event: React.ChangeEvent<HTMLInputElement>) { const file = event.target.files?.[0]; if (!file) return; setSyncing(true); const form = new FormData(); form.append('file', file); const response = await fetch(`/api/vehicles/${selectedVehicleId}/calendar`, { method: 'POST', body: form }); const data = await response.json(); setSyncing(false); setMessage(response.ok ? `Calendario sincronizado: ${data.imported} fechas importadas.` : data.error); if (response.ok) await loadData(selectedVehicleId); event.target.value = ''; }

  return <section className="mb-12 rounded-3xl border border-[#E9E1D2] bg-white p-6"><h2 className="font-serif text-2xl font-bold">Disponibilidad y tarifas</h2><p className="mb-5 text-xs text-[#6B726E]">Marca días no disponibles, añade precios por temporada y sincroniza calendarios externos.</p>{!vehicles.length ? <p className="text-sm text-[#6B726E]">Publica una camper para gestionar su disponibilidad.</p> : <><div className="mb-5 grid gap-4 lg:grid-cols-[minmax(220px,1fr)_2fr]"><label className="text-xs font-black uppercase tracking-wider text-[#6B726E]">Camper<select value={selectedVehicleId} onChange={(event) => setVehicleId(event.target.value)} className="mt-1 block w-full rounded-xl border border-[#E9E1D2] bg-white p-3 text-sm normal-case text-[#13322E]">{vehicles.map((vehicle) => <option key={vehicle.id} value={vehicle.id}>{vehicle.title}</option>)}</select></label><form onSubmit={addBlock} className="space-y-3"><DateRangeCalendar startDate={startDate} endDate={endDate} blocked={blocks} onChange={(start, end) => { setStartDate(start); setEndDate(end); }} /><button disabled={!startDate || !endDate} className="w-full rounded-full bg-[#13322E] px-4 py-3 text-xs font-bold text-white disabled:bg-slate-300">Marcar periodo como no disponible</button></form></div>
    <div className="mb-6 grid gap-4 lg:grid-cols-2"><form onSubmit={addRule} className="rounded-2xl border border-[#E9E1D2] bg-[#F7F6F2] p-4"><div className="mb-3 flex items-center gap-2"><DollarSign className="h-5 w-5 text-[#16B8AA]" /><strong className="text-sm">Tarifas por fechas <span className="font-normal text-[#6B726E]">({rules.length}/5)</span></strong></div><div className="grid gap-2 sm:grid-cols-2"><input required value={ruleName} onChange={(event) => setRuleName(event.target.value)} placeholder="Nombre: temporada alta" className="rounded-xl border border-[#E9E1D2] p-3 text-sm bg-white" /><input required type="number" min="10" step="0.01" value={rulePrice} onChange={(event) => setRulePrice(event.target.value)} placeholder="Tu precio: ej. 70 €/día" className="rounded-xl border border-[#E9E1D2] p-3 text-sm bg-white font-bold text-[#16B8AA]" /><input required type="date" value={ruleStart} onChange={(event) => setRuleStart(event.target.value)} className="rounded-xl border border-[#E9E1D2] p-3 text-sm bg-white" /><input required type="date" value={ruleEnd} onChange={(event) => setRuleEnd(event.target.value)} className="rounded-xl border border-[#E9E1D2] p-3 text-sm bg-white" />{Number(rulePrice) > 0 && <div className="sm:col-span-2 text-xs bg-white p-2.5 rounded-xl border border-teal-200 text-[#0F766E] font-medium flex items-center justify-between"><span>Viajero pagará: <strong>{(Number(rulePrice) * 1.045).toFixed(2)} €/día</strong></span><span>Tú recibirás: <strong>{(Number(rulePrice) * 0.90).toFixed(2)} € netos</strong></span></div>}</div><button disabled={rules.length >= 5} className="mt-3 rounded-full bg-[#16B8AA] px-4 py-2.5 text-xs font-bold text-white disabled:bg-slate-300 cursor-pointer">Añadir tarifa</button></form><div className="rounded-2xl border border-[#E9E1D2] p-4"><div className="flex items-center gap-2"><CalendarSync className="h-5 w-5 text-[#16B8AA]" /><strong className="text-sm">Sincronizar calendario</strong></div><p className="mt-2 text-xs text-[#6B726E]">Sube un archivo .ics exportado desde Booking, Yescapa u otro calendario. Sus eventos se marcarán como no disponibles.</p><label className="mt-3 inline-flex cursor-pointer items-center gap-2 rounded-full border border-[#E9E1D2] px-4 py-2.5 text-xs font-bold hover:border-[#16B8AA]"><Upload className="h-4 w-4" />{syncing ? 'Importando…' : 'Subir archivo .ics'}<input type="file" accept=".ics,text/calendar" className="hidden" disabled={syncing} onChange={importCalendar} /></label></div></div>
