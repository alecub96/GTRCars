  ...
  return <div className="min-h-screen bg-[#F7F6F2] text-[#13322E]"><Navbar />{paymentClientSecret && <StripePaymentElement clientSecret={paymentClientSecret} bookingId={id} onClose={() => setPaymentClientSecret('')} />}<main className="mx-auto max-w-6xl px-4 py-10">
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
      <section className="space-y-6">
        ...
        <article className="rounded-3xl border border-[#E9E1D2] bg-white p-6 shadow-sm"><div className="mb-4 flex items-center gap-3"><FileCheck2 className="h-6 w-6 text-[#16B8AA]" /><h2 className="font-serif text-2xl font-bold">Contrato de alquiler sin conductor</h2></div><div className="max-h-80 space-y-4 overflow-y-auto rounded-2xl bg-[#F7F6F2] p-5 text-sm leading-relaxed"><p><strong>1. Objeto.</strong> El propietario cede temporalmente el vehículo descrito para las fechas, horarios y lugar indicados en esta reserva. El viajero declara disponer de permiso de conducción válido.</p><p><strong>2. Precio y fianza.</strong> El precio total es de {booking.totalAmount} €. La fianza de {booking.depositAmount} € responde de daños, combustible, limpieza y kilometraje no incluidos, sin sustituir las coberturas del seguro.</p><p><strong>3. Uso y kilometraje.</strong> Se incluyen {booking.vehicle?.includedKmPerDay || 0} km por día. El exceso se factura a {booking.vehicle?.extraKmPrice || 0} €/km. No se permite conducción por personas no declaradas, uso ilícito, competición o cesión a terceros.</p><p><strong>4. Estado, entrega y devolución.</strong> Ambas partes documentarán kilometraje, combustible, limpieza, inventario y daños mediante check-in y check-out. El viajero debe comunicar inmediatamente cualquier accidente o avería.</p><p><strong>5. Cancelación.</strong> Política aplicable: {booking.vehicle?.cancellationPolicy || 'Standard'}. Los reembolsos y cargos se resolverán conforme a las condiciones aceptadas y al estado de la reserva.</p><p><strong>6. Normas del vehículo.</strong> {booking.vehicle?.rules || 'Uso responsable, devolución puntual y respeto del inventario entregado.'}</p><p><strong>7. Firma electrónica.</strong> La plataforma conserva la versión de condiciones, fecha, identidad autenticada y texto de firma como evidencia de aceptación. La firma no sustituye las verificaciones documentales obligatorias.</p></div>
        {!signed ? <div className="mt-5 space-y-3">{[['terms', 'He leído y acepto íntegramente el contrato y las normas del vehículo.'], ['deposit', `Comprendo la fianza de ${booking.depositAmount} € y los posibles cargos documentados.`], ['privacy', 'Acepto el tratamiento de datos necesario para gestionar esta reserva.']].map(([key, label]) => <label key={key} className="flex gap-3 rounded-xl border border-[#E9E1D2] p-3 text-sm"><input type="checkbox" checked={checks[key as keyof typeof checks]} onChange={(event) => setChecks({ ...checks, [key]: event.target.checked })} />{label}</label>)}<label className="block text-xs font-bold uppercase tracking-wider">Nombre completo como firma<input value={signature} onChange={(event) => setSignature(event.target.value)} placeholder={viewerRole === 'OWNER' ? `${booking.owner?.firstName || ''} ${booking.owner?.lastName || ''}` : `${booking.traveler?.firstName || ''} ${booking.traveler?.lastName || ''}`} className="mt-2 w-full rounded-xl border border-[#E9E1D2] p-3 text-base normal-case" /></label><button onClick={sign} disabled={loading} className="w-full rounded-full bg-[#13322E] py-4 text-sm font-bold text-white">{loading ? 'Guardando firma…' : `Firmar como ${viewerRole === 'OWNER' ? 'propietario' : 'viajero'}`}</button></div> : <div className="mt-5 rounded-2xl border border-green-200 bg-green-50 p-4 text-sm font-bold text-green-800"><CheckCircle2 className="mr-2 inline h-5 w-5" />Tu firma electrónica está registrada.</div>}
        <div className="mt-3 grid gap-2 text-xs sm:grid-cols-2"><p className={`rounded-xl p-3 font-bold ${booking.contract?.signedByTraveler ? 'bg-green-50 text-green-800' : 'bg-amber-50 text-amber-800'}`}>Viajero: {booking.contract?.signedByTraveler ? 'firmado' : 'pendiente'}</p><p className={`rounded-xl p-3 font-bold ${booking.contract?.signedByOwner ? 'bg-green-50 text-green-800' : 'bg-amber-50 text-amber-800'}`}>Propietario: {booking.contract?.signedByOwner ? 'firmado' : 'pendiente'}</p></div>
        </article>
      </section>
