...
export default async function BlogArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <div className="min-h-screen bg-[#FDFBF7] text-[#0F172A]"><Navbar /><main>
    <article className="max-w-4xl mx-auto px-4 py-12">
      ...
      <header className="mt-8 mb-10"><span className="text-xs font-black uppercase tracking-widest text-[#D97706]">{article.category} · {article.readingTime}</span>
        ...
        <p className="text-lg text-[#64748B] leading-relaxed">{article.excerpt}</p>
      </header>
      ...
      <div className="space-y-12">{article.sections.map((section) => <section key={section.heading}>
        ...
        {section.bullets && <ul className="mt-5 grid sm:grid-cols-2 gap-3">{section.bullets.map((item) => <li key={item} className="bg-white border border-[#E2E8F0] rounded-2xl px-4 py-3">✓ {item}</li>)}</ul>}
      </section>)}</div>
    </article>
    <aside className="bg-white border-t border-[#E2E8F0] py-14"><div className="max-w-4xl mx-auto px-4"><h2 className="font-serif text-3xl font-bold mb-7">Sigue preparando tu viaje</h2>
