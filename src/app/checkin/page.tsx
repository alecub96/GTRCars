...
function CheckInDigitalForm() {
  ...
  return <div className="min-h-screen bg-[#F7F6F2] text-[#13322E]"><Navbar /><main className="mx-auto max-w-3xl px-4 py-12"><section className="space-y-8 rounded-3xl border border-[#E9E1D2] bg-white p-6 shadow-xl sm:p-8">
    ...
    {existing ? <div className="py-10 text-center"><CheckCircle2 className="mx-auto h-16 w-16 text-emerald-600" /><h2 className="mt-4 font-serif text-2xl font-bold">Acta registrada correctamente</h2><p className="mt-2 text-sm text-[#6B726E]">Las partes de la reserva y administración pueden consultar las evidencias.</p></div> : booking && !booking.canSubmit ? <p className="rounded-2xl bg-amber-50 p-5 text-sm font-bold text-amber-900">El propietario debe completar el acta de entrega. Aquí podrás consultarla cuando quede registrada.</p> : booking && <form onSubmit={submit} className="space-y-6">
      <div className="rounded-2xl border border-[#16B8AA]/30 bg-[#F0FDFA] p-4 text-xs leading-relaxed"><ShieldCheck className="mr-2 inline h-5 w-5 text-[#16B8AA]" />Completa el acta en presencia de ambas partes. Se conserva fecha, usuario autenticado y fotografías.</div>
      <div className="grid gap-4 sm:grid-cols-2"><label className="text-xs font-black uppercase tracking-wider text-[#6B726E]">Kilometraje<input name="odometer" required type="number" min="0" max="10000000" className="mt-1 w-full rounded-xl border border-[#E9E1D2] p-3 text-sm" /></label><LevelSelect name="fuelLevel" label="Combustible" /><LevelSelect name="waterLevel" label="Agua limpia" /><CleanlinessSelect /></div>
      <EvidenceFields />
      <label className="block text-xs font-black uppercase tracking-wider text-[#6B726E]">Daños previos y observaciones<textarea name="notes" maxLength={3000} rows={4} className="mt-1 w-full rounded-xl border border-[#E9E1D2] p-3 text-sm normal-case" /></label>
      <button disabled={loading} className="w-full rounded-full bg-[#16B8AA] py-4 text-xs font-black uppercase tracking-widest text-white disabled:bg-slate-300">{loading ? 'Guardando acta…' : 'Registrar acta y evidencias'}</button>
    </form>}
  </section></main></div>;
}
...
export default function CheckInDigitalPage() { return <Suspense fallback={<div className="min-h-screen bg-[#F7F6F2]"><Navbar /></div>}><CheckInDigitalForm /></Suspense>; }
