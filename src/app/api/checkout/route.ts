...
export async function GET(request: Request) {
  try {
    ...
    return NextResponse.json({ success: true, booking: { code: booking.code, vehicle: booking.vehicle.title, status: booking.status, checkInOdometer: booking.checkIn?.odometer }, checkOut: booking.checkOut });
  } catch (error) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const booking = await prisma.booking.findUnique({ where: { id: bookingId }, select: { travelerId: true, ownerId: true, status: true, checkIn: { select: { odometer: true } }, checkOut: { select: { id: true } } } });
    ...
    const checkOut = await prisma.$transaction(async (tx) => {
      const created = await tx.checkOut.create({ data: { bookingId, odometer, fuelLevel, waterLevel, cleanliness, notes: notes || null, signed: true, photos: { create: [{ url: urls[0], tag: 'FRONT' }, { url: urls[1], tag: 'DASHBOARD' }, { url: urls[2], tag: 'INTERIOR' }] } }, include: { photos: true } });
