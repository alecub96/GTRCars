...
export async function POST(request: Request) {
  try {
    ...
    const license = form.get('fileLicense');
    ...
    if (
      ...
      return NextResponse.json(
        { error: 'Completa el documento, la fecha de expiración del permiso y adjunta las 3 fotos necesarias (Anverso DNI, Reverso DNI y Foto Carné de Conducir).' },
        { status: 400 }
      );
    }

    if (front.size > MAX_FILE_SIZE || back.size > MAX_FILE_SIZE || license.size > MAX_FILE_SIZE) {
      return NextResponse.json({ error: 'Cada archivo debe pesar menos de 5 MB' }, { status: 400 });
    }

    const allowed = ['image/jpeg', 'image/png', 'application/pdf'];
    if (!allowed.includes(front.type) || !allowed.includes(back.type) || !allowed.includes(license.type)) {
      return NextResponse.json({ error: 'Solo se admiten JPG, PNG o PDF' }, { status: 400 });
    }

    const [frontUrl, backUrl, licenseUrl] = await Promise.all([
      saveUpload(front, 'documents'),
      saveUpload(back, 'documents'),
      saveUpload(license, 'documents'),
    ]);
    ...
    await prisma.$transaction([
      prisma.document.create({
        data: {
          ...
          fileUrl: frontUrl,
          status: 'PENDING',
          notes: `Documento terminado en ${documentNumber.slice(-4)}; Caducidad permiso: ${licenseExpDate}`,
        },
      }),
      prisma.document.create({
        data: {
      ...
      prisma.document.create({
        data: {
          ...
          fileUrl: licenseUrl,
          status: 'PENDING',
          notes: `Foto Permiso de Conducir vinculada a DNI/NIE ${documentNumber}`,
        },
      }),
      prisma.user.update({
        where: { id: user.id },
